...
unsigned webserver::Request(void* ptr_s) {
  ...
  str_str << req.answer_.size();//Using chinese with utf-8,the content length is not correct.
  ...
  //Using chinese with utf-8,the content length is not correct.
  //s.SendLine("Content-Length: " + str_str.str());
  s.SendLine("");
  std::string content=EncodingConverter::ToUtf8String(req.answer_);
  s.SendLine(content);
